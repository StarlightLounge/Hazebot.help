...
from utils.logic import sync_member_roles, get_luck_bonus, elite_only
...
class HazeDex(commands.Cog):
    ...
    haze_group = app_commands.Group(name="hazedex", description="RPG: Collect and battle with unique smoke signatures")

    @haze_group.command(name="explore", description="Scan the vicinity for wild smoke and strains")
    @elite_only()
    ...
    @haze_group.command(name="catch", description="Attempt to synchronize with the active smoke")
    @elite_only()
    async def catch(self, interaction: discord.Interaction):
        if interaction.guild_id not in self.active_spawns:
            return await interaction.response.send_message("❌ No active signals detected. Use `/hazedex explore`.", ephemeral=True)
    ...
    @haze_group.command(name="battle", description="Battle a subject with your HazeDex collection")
    @app_commands.describe(opponent="The subject to challenge", your_item="The item from your collection to use")
    @elite_only()
    async def battle(self, interaction: discord.Interaction, opponent: discord.Member, your_item: str):
        ...
        if p1_pwr > p2_pwr:
            ...
            db.update_user(interaction.user.id, interaction.guild_id, {"hash_coins": user_data.get("hash_coins", 420) + reward})
        elif p2_pwr > p1_pwr:
            reward = 250
            embed.description = f"🏆 **{opponent.name}** dominated the session!\n**Reward:** `250 H$`"
            db.update_user(opponent.id, interaction.guild_id, {"hash_coins": opp_data.get("hash_coins", 420) + reward})
        else:
            embed.description = "🌫️ **DRAW.** The room is too thick to determine a victor."
            
        await interaction.response.send_message(embed=embed)

    @haze_group.command(name="view", description="Access your collection of smoke and strains")
    @app_commands.describe(member="The subject whose collection to view")
    @elite_only()
        ...
        if collection:
            ...
            for item_name in collection:
                ...
                if item_data:
                    ...
                    sorted_coll[t].append(item_name)
            ...
            for t, items in sorted_coll.items():
                ...
                for name, count in Counter(items).items():
                    display_items.append(f"`{name}` x{count}" if count > 1 else f"`{name}`")
    ...
    @haze_group.command(name="registry", description="Access the master registry of smoke signatures")
    @elite_only()
    ...
    @haze_group.command(name="trade", description="Initiate a neural swap with another subject")
    @app_commands.describe(target="The subject to trade with", your_item="Item to give", their_item="Item to receive")
    @elite_only()
    async def trade(self, interaction: discord.Interaction, target: discord.Member, your_item: str, their_item: str):
